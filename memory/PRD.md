...
## Last Updated
2026-06-30 (7): Verträge im Panel editierbar + Startdatum + Überspringen. (A) Vertragsvorlagen in DB-Collection `contract_templates` (auto-seed aus `_build_contract_html_parts` mit Platzhalter `{{START_DATE}}`). Admin-Editor in Einstellungen → Arbeitsverträge: Titel/Untertitel/Position + Freitext-HTML des Vertrags bearbeiten, Vorschau + Speichern (global). Endpoints in applications.py: GET/PUT `/api/applications/contract-templates[/type]` (admin-only via role-check). Bewerber-Signaturseite (MitarbeiterContractSign) rendert Body jetzt aus DB (dangerouslySetInnerHTML) via neuem `GET /api/applications/my-contract`. Auch die PDF (download-contract) nutzt das DB-Template. (B) Custom Startdatum: beim Annehmen (Accept-Dialog: Datumsfeld) → `contract_start_date`, ersetzt `{{START_DATE}}` in §1. (C) Überspringen: Accept-Dialog-Checkbox → `contract_can_skip`; Bewerber sieht „Später unterschreiben"-Button → `POST /api/applications/skip-contract` (setzt Status „Vertrag unterschrieben" + `contract_skipped=true`), gelangt weiter zur Verifizierung. Verifiziert: curl (Templates GET/PUT, my-contract Startdatum-Ersetzung, skip) + Testing-Agent iteration_24 (Editor speichern/persistieren/zurücksetzen, Accept-Dialog-Felder) 100%. HINWEIS (optional): Accept-Dialog nutzt natives date-input; könnte auf shadcn DatePicker umgestellt werden.

2026-06-30 (6): Admin-Einstellungsseite `/admin/settings` mit Tab „Arbeitsverträge" (jetzt Editor, s.o.).
