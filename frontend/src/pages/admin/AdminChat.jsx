...
import { MessageCircle, Send, Search, ArrowLeft, User, Clock } from 'lucide-react';
...
const AdminChat = () => {
  ...
  // Merge employees with conversation data
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      {convo && (
                        <span className="text-xs text-gray-400 flex-shrink-0 ml-2">{formatDate(convo.last_time)}</span>
                      )}
                    </div>
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-gray-500 truncate">
                        {convo
                          ? `${convo.last_sender_role === 'admin' ? 'Du: ' : ''}${convo.last_message}`
                          : emp.position || emp.email
                        }
                      </p>
                      {convo?.unread > 0 && (
                        <span className="ml-2 px-1.5 py-0.5 bg-emerald-500 text-white text-xs rounded-full min-w-[20px] text-center flex-shrink-0">
                          {convo.unread}
                        </span>
                      )}
                    </div>
        ...
        {selectedPartner ? (
          <>
            ...
            <div className="flex items-center gap-3 p-4 border-b border-gray-100">
              <button
                onClick={() => { setSelectedPartner(null); fetchEmployees(); }}
