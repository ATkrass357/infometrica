...
## Last Updated
2026-06-30 (3): KI-Generator für App-Test-Aufgaben. Button „✨ Mit KI generieren" im Aufgaben-Formular (nur bei Kategorie App Test). Admin gibt App-Namen ein → Gemini 3 Flash (gemini-3-flash-preview via emergentintegrations + EMERGENT_LLM_KEY) erzeugt Titel + Einleitung + Schritt 1–3 auf Deutsch. Duplikat-Sperre: für eine App wird nie zweimal eine Aufgabe erzeugt (Backend prüft `ai_app_name` case-insensitive gegen bestehende app-Tasks → 409). Kein Live-Web-Fetch (nur LLM-Wissen, so vom Nutzer gewünscht). Kosten: ~640 Tokens/Aufgabe (~180 in / ~460 out). Backend: `services/ai_task_service.py`, `POST /api/admin/tasks/ai-generate`, Task/TaskCreate um `ai_app_name` erweitert. Frontend: AdminTasks.jsx (generateWithAI). Testing-Agent iteration_22: 5/5 Szenarien bestanden (100%).
